...
def run_simulation(failure_rate=0.0, num_nodes=NUM_NODES, steps=100, event_start=25, event_end=45):
    ...
    print(f"Failure rate: {failure_rate*100:.0f}%, Alive: {len(alive_nodes)}/{num_nodes}")
    print("Step,RegionalRisk,Alarm")
    ...
    for step in range(steps):
        ...
        alarm = regional_risk > theta_adaptive
        
        # Ghi log
        print(f"{step},{regional_risk:.4f},{alarm}")
        
        if alarm and first_alarm_step is None:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 simulate_latency.py <failure_rate>")
        sys.exit(1)
