...
def extract_accuracy_from_lines(lines):
    event_lines = []
    for line in lines:
...
def parse_log_file(filename):
    """Đọc file log, trả về danh sách accuracy các run."""
    ...
    for line in lines:
        if line.startswith("NodeID"):
            if current_run:
                acc = extract_accuracy_from_lines(current_run)
                runs.append(acc)
                current_run = []
        else:
            parts = line.strip().split(',')
...
def main():
    protocols = {
        'ours': ('all_runs_v3_fr{:.1f}.log', 30),
        'tbcp': ('all_runs_tbcp_fr{:.1f}.log', 30),
        'leach': ('all_runs_leach_fr{:.1f}.log', 30)
    }
    levels = [0, 20, 40, 60]
    all_data = []
    run_counter = 1

    for proto, (pattern, expected) in protocols.items():
        for lvl in levels:
