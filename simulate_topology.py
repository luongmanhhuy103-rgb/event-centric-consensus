...
def run_simulation(num_nodes, failure_rate, steps=100, event_start=25, event_end=45):
    nodes = [Node(i, failure_rate) for i in range(num_nodes)]
    alive_nodes = [n for n in nodes if not n.dead]
    if len(alive_nodes) < 2:
        return 0.0

    broadcasts_in_event = 0
    for step in range(steps):
        ...
        for node in nodes:
            if node.step(event):
                any_broadcast = True
        if any_broadcast and event:
            broadcasts_in_event += 1

    total_event_steps = event_end - event_start + 1
    return (broadcasts_in_event / total_event_steps) * 100 if total_event_steps > 0 else 0.0

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 simulate_topology.py <num_nodes> <failure_rate>")
        sys.exit(1)
    num_nodes = int(sys.argv[1])
    failure_rate = float(sys.argv[2])
    acc = run_simulation(num_nodes, failure_rate)
